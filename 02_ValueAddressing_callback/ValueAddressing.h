...
using AddressWrite_Callback = std::function<uint32_t(uint64_t address, uint8_t* buf2Write, size_t size)>;
using AddressRead_Callback = std::function<uint32_t(uint64_t address, uint8_t* buf2Read, size_t size)>;
...
public:
    ...
    void operator= (StructType _structObject){
        address = _structObject.getAddress();
        if(!_structObject.getWriteCallback()){
            _structObject.setCallback(structObject.getWriteCallback(), structObject.getReadCallback());
            _structObject.setValuesAddress(address);
        }
    ...
    operator StructType(){
        return (StructType)structObject;
    }
    ...
    void setCallback(AddressWrite_Callback _toWrite, AddressRead_Callback _toRead){
        structObject.setCallback(_toWrite, _toRead);
    }
private:
...
protected:
    AddressWrite_Callback toWrite = nullptr;
    AddressRead_Callback toRead = nullptr;
    uint64_t address = 0;
};
...
public:
    ...
    void operator= (DataType _value){
        value = _value;
        int ret = toWrite ? toWrite(address, (uint8_t*)&value, size()) : -1;
    }
    // get value
    operator DataType(){
        int ret = toRead ? toRead(address, (uint8_t*)&value, size()) : -1;
        return value;
    }
    // get address
    uint64_t operator& (){
...
public:
    ...
    void valueSet(ValueAddressingBase* value){
        value->setCallback(toWrite, toRead);
    ...
    void structSet(T* structValue){
        structValue->setCallback(toWrite, toRead);
    ...
    void arraySet(T array, size_t n){
        ...
        for(size_t i = 0; i < n; ++i){
            base = array[i];
            base->setCallback(toWrite, toRead);
    ...
    AddressWrite_Callback getWriteCallback(){
        return toWrite;
    }
    AddressRead_Callback getReadCallback(){
        return toRead;
    }
